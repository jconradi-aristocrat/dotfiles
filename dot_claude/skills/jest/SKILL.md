...
Best Practices
Write descriptive test names - Use clear, specific names that explain what the test verifies
Follow AAA pattern - Structure tests with Arrange, Act, Assert sections for clarity
Test behavior, not implementation - Focus on what the code does, not how it does it
Use appropriate matchers - Choose the most specific matcher for better error messages
Mock external dependencies - Isolate unit tests from external services and modules
Clean up after tests - Use afterEach to reset mocks and clean up side effects
Avoid testing private methods - Test public interfaces and trust implementation details
Use setup and teardown hooks effectively - Leverage beforeEach/afterEach for common setup
Test edge cases and error conditions - Don't just test happy paths
Keep tests fast and isolated - Each test should run independently and quickly
Common Pitfalls
Not cleaning up mocks between tests - Leads to test pollution and flaky tests
Testing implementation details - Makes tests brittle and hard to maintain
Overusing snapshots - Snapshots should supplement, not replace, specific assertions
Forgetting to await async operations - Results in false positives and flaky tests
Not using proper matchers - Generic matchers provide poor error messages
Mocking too much - Over-mocking reduces test confidence and value
Writing integration tests as unit tests - Mixing concerns makes tests slow and complex
Not testing error scenarios - Only testing happy paths leaves bugs uncaught
Shared state between tests - Global variables and singletons cause test interdependence
Unclear test names - Vague names make it hard to understand test failures
